...
const utils = {
    logDir: path.join(os.homedir(), 'AppData', 'Local', 'Cortex', 'Logs'),
    // function to initialize the logger used in the entire project
    createLogger: () => {

        // Ensure the logs directory exists
        if (!fs.existsSync(utils.logDir)) {
            fs.mkdirSync(utils.logDir, { recursive: true });
        }
        ...
        const logger = createLogger({
            ...
            transports: [
                ...
                new _transports.DailyRotateFile({
                    filename: path.join(utils.logDir, 'memgptbroker-%DATE%.log'),
